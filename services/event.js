...
const eventService = {
  ...
  // Upcoming events first (soonest at top), then undated events, then past events.
  async getAll() {
  ...
  // Returns a structured result so the route can distinguish "not found" from "no image".
  async delete(event_id) {
    const [rows] = await pool.execute(
      `SELECT image_path FROM events WHERE event_id = ?`, [event_id]
    );
    if (!rows.length) return { found: false, imagePath: null };
    await pool.execute(`DELETE FROM events WHERE event_id = ?`, [event_id]);
    return { found: true, imagePath: rows[0].image_path || null };
  },
};
